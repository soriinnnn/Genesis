...
void MainMenu::setupScene() 
{
	...
	spaceshipTransform->setPosition({-16.0f, 0.0f, 10.0f});
	spaceshipTransform->setRotation({-0.32f, -1.33f, 0.0f});

	MeshRendererComponent* mesh = m_spaceship->createComponent<MeshRendererComponent>();
	mesh->setMesh(m_context.resources.getMesh(ASSETS_SPACESHIP_MESH));
	mesh->setMaterial(m_context.resources.getMaterial(ASSETS_SPACESHIP_MATERIAL));
}