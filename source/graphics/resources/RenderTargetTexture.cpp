...
RenderTargetTexture::RenderTargetTexture(const RenderTargetTextureDesc& tdesc, const GraphicsResourceDesc& gdesc): GraphicsResource(gdesc), m_size{tdesc.size}, m_format{DXGI_FORMAT_R8G8B8A8_UNORM}, m_sampleCount{tdesc.sampleCount}
{
	...
	D3D11_TEXTURE2D_DESC textureDesc = getD3D11TextureDesc(m_size, m_format, m_sampleCount, sampleQualityLevel);
	GENESIS_GRAPHICS_LOG_THROW_ON_FAIL(
	...
	GENESIS_GRAPHICS_LOG_THROW_ON_FAIL(
		m_device.CreateShaderResourceView(m_texture.Get(), nullptr, &m_resourceView),
		"CreateShaderResourceView failed."
	);

	GENESIS_GRAPHICS_LOG_THROW_ON_FAIL(
		m_device.CreateRenderTargetView(m_texture.Get(), nullptr, &m_targetView),
